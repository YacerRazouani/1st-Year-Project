...
void verifierMurDroit()
{
    ...
    if (valeurCMDroit >= 24 && valeurCMDroit <= 60)
    {
       ...
       uint8_t distanceAParcourir = (valeurCMDroit - 18);
        ...
        uint8_t temps = round((distanceAParcourir * 1000) / 17);    // 15cm/s vitesse
...
void verifierMurGauche()
{
    ...
    if (valeurCMGauche >= 24 && valeurCMGauche <= 60)
    {
        ...
        uint8_t distanceAParcourir = (valeurCMGauche - 20);
        //moteur.ajustementPWM(180, 160);
        uint16_t temps = round((distanceAParcourir * 1000)/ 19);    // 15cm/s vitesse
        moteur.ajustementPWM(180, 160);        
